...
/**
 * The performing unit's dashboard for one order category.
 *
 * `category` narrows it to laboratory or imaging (see LAB_CATEGORY /
 * IMAGING_CATEGORY in use-clinical). Omitted, it counts every order, which is
 * what the laboratory saw before the radiographer role existed. The queryKey
 * includes the category so the two do not share a cache entry.
 */
export function useLabDashboard(category?: string) {
  const scope = category ? { category } : {}
  return useQuery({
    queryKey: ["lab-dashboard", category ?? "all"],
    queryFn: async (): Promise<LabDashboardData> => {
      const { start } = todayRange()
      const [pendingOrders, reportsToday, reportsTotal, orders] = await Promise.all([
        countIfPermitted("ServiceRequest", { ...scope, status: "active" }),
        countIfPermitted("DiagnosticReport", { ...scope, issued: `ge${start}` }),
        countIfPermitted("DiagnosticReport", scope),
