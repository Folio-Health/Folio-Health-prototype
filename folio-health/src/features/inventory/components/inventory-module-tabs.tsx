...
const LINKS = [
  { href: "/inventory", label: "Supplies" },
  { href: "/inventory/equipment", label: "Equipment" },
  { href: "/inventory/warehouse", label: "Warehouse" },
  { href: "/inventory/purchase-requests", label: "Purchase Requests" },
  { href: "/inventory/stock-movement", label: "Stock Movement" },
]

function InventoryModuleTabs() {
  const pathname = usePathname()

  return (
    <nav className="mb-6 flex w-fit max-w-full flex-wrap items-center gap-1 rounded-lg bg-muted p-0.75">
      {LINKS.map((link) => {
        const isActive = link.href === "/inventory" ? pathname === link.href : pathname?.startsWith(link.href)
        return (
          <Link
            key={link.href}
            href={link.href}
